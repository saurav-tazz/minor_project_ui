...
function createPlayer(userId, genres) {
  ...
  socket.on("matchFound", (data) => {
    ...
    console.log(`${userId} matched with ${data.opponentId} in room: ${data.roomName} (Similarity: ${data.similarity})`);
    ...
    setTimeout(() => {
      ...
      // Now, simply emit endMatch without the 'requeue' flag
      socket.emit("endMatch", { matchId: socket.matchId }); 
    }, 2000);

    socket.on("newQuestion", (data) => {
  console.log(`${userId} received question:`, data.question);
...
socket.on("gameOver", (data) => {
  console.log(`${userId} GAME OVER`, data);
});

  });
socket.on("scoreUpdate", (data) => {
  console.log(`${userId} scores:`, data);
});

  socket.on("matchEnded", ({ matchId }) => {
    console.log(`--- ${userId} MATCH ENDED ---`);
    console.log(`${userId}'s match ${matchId} has ended. Player is now in the lobby.`); // Adjusted message
    
    delete socket.matchId;
    delete socket.roomName;
    // In a real app, this is where the player would typically return to a main lobby screen
    // or a "game over" screen without automatically searching for a new match.
  });

  socket.on("opponentDisconnected", ({ matchId, opponentId }) => {
    console.log(`!!! ${userId} OPPONENT DISCONNECTED!!!`);
    console.log(`${userId}'s opponent ${opponentId} disconnected in match ${matchId}. Player is now in the lobby.`); // Adjusted message
